...
本索引覆盖 2026-09-04–05 从 GitHub 账户 `ziyu24` 枚举并重新核对的 75 个既有仓库，并追加 2026-09-05 新建的私有证据仓 `cqc_naood`，当前共 76 个仓库，其中 61 个为私有仓库、15 个为公开仓库。审计抓取了既有各库全部远端分支，以 `main`/`master` 为主要分支，并比较提交时间更新的次要分支；`cqc_P1`、`cqc_P3`、`cqc_P4`、`cqc_P5` 和 `cqc_research` 的次要分支较新，表中同时记录比较边界。停止维护的旧项目同样只读审计，没有修改或运行其内容。
