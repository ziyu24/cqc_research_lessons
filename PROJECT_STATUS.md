...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-25 | B已独立核验同16000父态四臂原始更新、验证、成本及八份完整权重：keep/关闭/半量/旧教师回退59.6546%/57.3584%/59.3974%/59.1183%。半量阶段省13.70%但低0.2573点；keep/半量相对父态仅+0.3100/+0.0527点。已交付双卡固定模型图像组成诊断，零新训练；不将恢复差异归为已识别机制，不设事后非劣界限。原同质量总成本目标未达成，自适应路线尚未实现；历史局部正信号保留。[来源证据](https://github.com/ziyu24/cqc_P24/blob/b1e78376c5e72254eb13ff262f8c7f1555829791/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/b1e78376c5e72254eb13ff262f8c7f1555829791/lab/discussion.md)。 |
...
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-26 | 弱框原位保护相对同面积错位/全图的源AP50差−1.620/−2.352点、既有外域+0.080/+0.286点，未通过预设联合条件；完整实际状态、双rank、独立预测和源指标重放已核验，停止该固定配置扩展。普通增强已有有限正结果保留；原目标未达成，一般背景路线及因果命题未知。冻结输出错误分解已交付，仅CPU统计，尚未正式完成。[当前问题](https://github.com/ziyu24/cqc_P26/blob/699b42d37a738a96477954fe08be00fdac4bbe56/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/699b42d37a738a96477954fe08be00fdac4bbe56/lab/result.md)。 |
