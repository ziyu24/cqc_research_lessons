...
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-25 | 普通外观增强与等量续训的旧实现对照已完成，连续AP50源/外域差−0.726/−0.058点；其它指标有正有负。复核真实矩形输入发现共同教师选样层界错误，旧观察保留，已交付同父状态的修正短续训并待补齐；父前缀仍含历史实现限制，不把错误当作负差值已识别根因。原目标未达成，背景路线仍不可判，项目继续运行。[问题与下一步](https://github.com/ziyu24/cqc_P26/blob/ce716d350b4cc7fd0a24f400ff2da32a9677ea0c/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/ce716d350b4cc7fd0a24f400ff2da32a9677ea0c/lab/result.md)。 |
