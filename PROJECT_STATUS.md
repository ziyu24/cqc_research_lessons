...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-25 | 较早共同12800父态的四格和关闭控制已独立核验：旧半量/旧全量末点60.2546%/60.2006%，半量阶段命令省13.5291%；刷新在两种计算量下均从中间点优势转为终点落后。已交付同16000完整状态的重启控制、关闭、减半和换回旧预测器四动作，尚无新结果。三条教训保留，未识别原退化原因；自适应路线尚未实现，原同质量总成本目标未达成。[原问题与当前计划](https://github.com/ziyu24/cqc_P24/blob/b530f160df60502ef671ecb3f93cd82586ae92f4/lab/discussion.md)；[已核验证据](https://github.com/ziyu24/cqc_P24/blob/b530f160df60502ef671ecb3f93cd82586ae92f4/lab/result.md)。 |
| P25（`cqc_P25`） | 没有完整OBB标注源域，仅有部分HBox或Point图像与完全无标签图像，如何学习对训练不可见目标域泛化的旋转检测器。 | 正在运行 | 原问题与用户授权不变。普通多尺度诊断域AP50从11.461到15.550；固定一致性筛选15.675，对置信度/随机增量未达推进条件但未触发严格否定，结论未定。三臂及完整预测已独立核验；已有HBox的冻结信号诊断已交付、零训练且尚无结果。目标未达成，跨种子/严格盲域未验证。[已核证据](https://github.com/ziyu24/cqc_P25/blob/ae5a6951e42f4a46c1d2c439e508ad733bb6c9b0/lab/result.md)、[当前问题](https://github.com/ziyu24/cqc_P25/blob/ae5a6951e42f4a46c1d2c439e508ad733bb6c9b0/lab/discussion.md)。 | 原问题与用户授权不变。共同初态尺度及L/U位置消融已完整核验，HBox端缩放贡献较大；普通多尺度诊断域AP50从11.461提高到15.550。目标未达成，可靠性机制及等配额置信度/随机对照已交付但未训练，跨种子和严格盲域未验证。[已核证据及边界](https://github.com/ziyu24/cqc_P25/blob/4d6024673013b3a137d9749e92ffea2103ff74bc/lab/result.md)、[当前问题](https://github.com/ziyu24/cqc_P25/blob/4d6024673013b3a137d9749e92ffea2103ff74bc/lab/discussion.md)。 |
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-25 | 修正共同矩形层界后的同父短续训已核验，普通增强相对等量原样的源/既有外域连续AP50差+3.362/+0.498点，达到有限正结果；AP75略降且外域误报增加，外域绝对AP50仍仅3.653。原目标未达成，背景机制未知；共同父前缀及单seed限制保持。已交付两固定模型的源域区域敏感性比较，仅评测、尚无结果。[当前问题](https://github.com/ziyu24/cqc_P26/blob/fab6bc541b2f7ab634b62ff28cc8ef68c853181b/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/fab6bc541b2f7ab634b62ff28cc8ef68c853181b/lab/result.md)。 |
