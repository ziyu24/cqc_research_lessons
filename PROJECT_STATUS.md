...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-26 | 旧视图余弦排序在三状态×两教师的新视图方向目标上均优于等配额哈希，相关0.42—0.51；低loss优先全部落后。实际输入、权重、两rank及完整选择/标量统计已独立复算一致，耗时0.65115 GPU小时。已交付同19200完整父态、缓存余弦与哈希各3200步的真实训练对照；每臂实际两卡，保留四逻辑批及全局batch12、原生AdamW/EMA，GPU首批仍待验证。尚无训练增益，原同质量总成本目标未达成，自适应联合路线尚未实现。[来源证据](https://github.com/ziyu24/cqc_P24/blob/2655174566af76d7f959ec0be42054012997396a/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/2655174566af76d7f959ec0be42054012997396a/lab/discussion.md)。 |
| P25（`cqc_P25`） | 没有完整OBB标注源域，仅有部分HBox或Point图像与完全无标签图像，如何学习对训练不可见目标域泛化的旋转检测器。 | 正在运行 | 2026-09-26 | 原目标不变。普通多尺度收益保留；固定校准/供体规则停止。原生U框回归删除/保留两臂已完整重算，删除的目标AP50−1.179点而召回上升，源AP50+4.898点，双方均未达联合条件，停止固定bbox系数/步数/种子搜索，不否定全部U。未选候选背景分类惩罚删除的单臂训练已交付，复用同机对照，尚无新性能；项目目标未达成。[来源证据](https://github.com/ziyu24/cqc_P25/blob/0ca20fa3dc6e50ddfc092e553384646a2a1dc5cf/lab/result.md)、[当前问题](https://github.com/ziyu24/cqc_P25/blob/0ca20fa3dc6e50ddfc092e553384646a2a1dc5cf/lab/discussion.md)。 |
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-26 | 固定负分类降权完整核验：源AP50−1.204、DIOR+0.565点，必要联合条件失败，按预设未运行整体缩放控制。保留外域/AP75正差，停止该常数搜索；固定空间配置失败、普通增强有限正结果保持，原目标未达成，背景机制未知。教师条件化负项权重与逐图逐层错位对照已交付，单seed两卡，尚无性能结果。[当前问题](https://github.com/ziyu24/cqc_P26/blob/2c3747580786b1fdf8d518023418dc554652fc1e/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/2c3747580786b1fdf8d518023418dc554652fc1e/lab/result.md)。 |
