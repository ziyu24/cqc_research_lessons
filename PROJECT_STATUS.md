...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-26 | B已独立重算五固定模型的完整预测、原生标签及全部配对抽样/删图。暂停减继续为−2.2962点，逐一删图始终负；半量减继续−0.2573点及教师回退差可随删图变号，不能宣称非劣或稳定规则。半量阶段省13.70%的事实保留，原同质量总成本目标未达成，自适应路线尚未实现。已交付三个固定学生状态的训练可见信号诊断，两卡、零优化，仅用既有训练HBox参照；尚无候选信号有效性结果。[来源证据](https://github.com/ziyu24/cqc_P24/blob/767ef10f2d440038b6d2df6f3b6eccd29237df3d/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/767ef10f2d440038b6d2df6f3b6eccd29237df3d/lab/discussion.md)。 |
