...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-26 | 同一完整父态、单seed和相同每步四选二预算下，缓存余弦61.4982%，哈希60.4413%，高1.0569点。实际数据/两卡四逻辑rank、完整出现流、原生验证、四份完整权重和全部尝试已独立核验，保存统计重算一致；本轮17.0883 GPU小时。缓存取得计入后成本仍更高，尚未证明同质量总成本下降或自适应联合成功。用户离线期间的固定队列已交付：全量对照、一次固定评分置换半量及四终点图像组成诊断；两项训练均单seed/实际两卡，旧两半量复用，尚无新结果，原目标不变。[来源证据](https://github.com/ziyu24/cqc_P24/blob/36c098d75c94f71f353b19738c15b2897732ca9c/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/99fc241c9825914610b63184846267b3b23cb405/lab/discussion.md)。 |
