...
| P25（`cqc_P25`） | 没有完整OBB标注源域，仅有部分HBox或Point图像与完全无标签图像，如何学习对训练不可见目标域泛化的旋转检测器。 | 正在运行 | 2026-09-25 | 原问题与授权不变。普通多尺度诊断域AP50从11.461到15.550；固定一致性筛选仍未定。固定线性评分器及跨尺度供体规则均未达到既定推进幅度，完整独立重算通过，停止各自固定机制的训练投入，保留局部正差和一般可靠性未知。原生U框回归删除/保留的同起点同机对照已交付，尚无训练结果；项目目标未达成。[来源证据](https://github.com/ziyu24/cqc_P25/blob/2b42a6fcb24b907b0fa05283fc630f47bc3569ba/lab/result.md)、[当前问题](https://github.com/ziyu24/cqc_P25/blob/2b42a6fcb24b907b0fa05283fc630f47bc3569ba/lab/discussion.md)。 |
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-26 | 原生候选链完整核验：DIOR原样/全图增强解码即无覆盖15,665/15,504个，类别阈值首次损失10,911/9,942个，截断损失为零。几何与筛选共同限制召回，训练根因未知；固定空间配置失败及普通增强有限正结果保留，原目标未达成。已交付原生负分类降权与整体缩放的固定单seed两卡对照，尚无性能结果。[当前问题](https://github.com/ziyu24/cqc_P26/blob/68fce8d7ef880e261f556a6da5122f85bb093e84/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/68fce8d7ef880e261f556a6da5122f85bb093e84/lab/result.md)。 |
