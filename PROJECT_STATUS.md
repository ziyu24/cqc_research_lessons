...
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-25 | 用户已确定问题、允许部分图完整弱标并授权继续研究。固定单种子的完整/稀疏HBB对照已完成：HRSC源域AP50为95.380/82.540，DIOR-R为21.658/3.052，漏标额外跨域绝对下降5.766个百分点。原目标未达成，背景机制和因果主张未知；冻结源模型的背景敏感性诊断已交付、未执行，不自动扩展训练。[原问题与授权](https://github.com/ziyu24/cqc_P26/blob/1df2d4cf5925935811ec8439f6ed3ee4b662e0d2/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/1df2d4cf5925935811ec8439f6ed3ee4b662e0d2/lab/result.md)。 |
