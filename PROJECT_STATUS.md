...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-26 | 固定三状态的训练可见信号诊断已独立核验输入、权重、两卡记录及保存标量统计，耗时0.72235 GPU小时。loss/CPF与HBox参照方向相关较弱且随状态变化；未检验阈值或未来AP，撤回单阈值方法失败的过强裁决。已交付旧记录跨增强排序与教师选择诊断，两卡、零优化；尚无迁移结果。原同质量总成本目标未达成，自适应路线尚未实现，既有局部省时事实保留。[来源证据](https://github.com/ziyu24/cqc_P24/blob/92f573aa6c18b09c3c9016d8c04886fd8be7f7cd/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/92f573aa6c18b09c3c9016d8c04886fd8be7f7cd/lab/discussion.md)。 |
