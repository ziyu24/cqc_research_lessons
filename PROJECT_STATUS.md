...
| P24（`cqc_P24`） | 在部分弱监督旋转检测中，区分继续学习、刷新监督与暂时跳过的边际收益，降低达到同等检测质量的总计算成本。 | 正在运行 | 2026-09-26 | 同父态、单seed的两卡匹配半量中，缓存余弦61.4982%，哈希60.4413%，高1.0569点；实际数据、完整出现流、原生验证、权重及17.0883 GPU小时已核验。该局部AP事实保留，但两卡共载四rank/Gloo/激活重算改变原四卡执行条件，不能与历史四卡成本归因比较。按用户明确要求撤回待执行两卡安排，交付每臂四张物理GPU、原生NCCL及无额外重算的全量、缓存半量、哈希半量、一次固定评分置换，另核四卡缓存构建和四终点图像组成；尚无新结果。原同质量总成本目标未达成，自适应联合未成功，命题未被整体证伪。[来源证据](https://github.com/ziyu24/cqc_P24/blob/1911265fe04aba59f540b94e70a48c2447c6f7a7/lab/result.md)；[当前问题](https://github.com/ziyu24/cqc_P24/blob/1911265fe04aba59f540b94e70a48c2447c6f7a7/lab/discussion.md)。 |
...
| P26（`cqc_P26`） | 全部源图仅有少量HBB或点、实例整体不完整时，能否学习对未知域泛化的旋转检测器，并提炼跨域通用的因果本质特征。 | 正在运行 | 2026-09-26 | 固定教师条件化负项权重完整核验：源AP50−1.725、DIOR+0.358点，必要联合条件失败，按预设跳过位置对照。统一降权及固定空间配置的有界失败、普通增强的有限正结果保持，背景根因未知，原目标未达成。停止该固定方案及局部搜索，当前无新增训练任务，项目未结项。[当前问题](https://github.com/ziyu24/cqc_P26/blob/208e48e9e2435604f9efd93e8c8989eeb7810640/lab/discussion.md)、[已核结果](https://github.com/ziyu24/cqc_P26/blob/208e48e9e2435604f9efd93e8c8989eeb7810640/lab/result.md)。 |
