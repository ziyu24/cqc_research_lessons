...
- **事实**：任意共同漏检/偏差下，单靠预测联合分布不能识别完整真实风险；项目文献审查认为已提出的自适应、post-selection 和 structured-output 组合缺少独立贡献，这不是对所有后续组合的普适否定。
- **事实**：FlightShift 原计划需要至少一万张穷尽 OBB 标注关键帧（不是一万个实例）与双人 QA，和当时单研究者、零标注预算直接冲突。
- **推断**：该仓库的价值是及时停止伪新颖或不可执行的问题，而不是一个失败算法。
- **未知**：资源变化或出现新问题、非平凡理论/效率进展时可重新审计；不要求一切贡献都必须使用现有框架不能表达的 estimand，但不能只换名称继承新颖性主张。
...
- 后续做法：先写出对现有定理的直接归约及其假设，再判断新依赖、决策、效率或真实应用证据是否产生非平凡贡献；不以框架名称代替逐命题比较。
...
- 失败命题：依赖至少一万张穷尽标注关键帧与双人 QA 的新采集基准，可以在单人、零新增预算条件下作为近期主线。
