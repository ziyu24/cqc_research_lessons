...
审计基线：当前主线 `ziyu24/D7_pcp_obb_foundation@fa448ea9498d8c21cea4560d40fffa8c049623b2`。已抓取并检查全部远端分支，没有更新更晚的次线。最新纸面审查确认矩形查询族的同尺度包含命题成立，但预测标准化后精确归约到已有同时共形带；旧尺度运输的支配不能直接移植。当前没有新算法或部署增量，不启动真实数据验证。此前主动审核归约、同尺度基线与计数查询支配证据保留；旧原型保持停止，已有开发样本上的复算仍是事后诊断，不是独立确认。

快速阅读路径：先读“项目研究什么”→“教训十三、十四、十五、十六”→“教训一、二、三、八、九、十、十一、十二”→“方法族停止索引”。
