...
审计基线：当前主线 `ziyu24/D7_pcp_obb_foundation@7dd70a32696510674ce04afecd51a214b2a96e38`。已抓取并检查全部远端分支，没有更新更晚的次线。最新研究核对 OBB 可靠性基准的一手近邻、现有输出和适配合同：原图 ID 闭合不等于全部 operator 与类别输入可恢复，统一风险名称也不保留原生统计条件。完整提议暂不下发，benchmark 实用性仍未知。此前查询族/主动审核归约与计数基线证据保留；旧原型继续停止，已有开发样本复算仍不是独立确认。

快速阅读路径：先读“项目研究什么”→“教训十七、十八、十三、十四、十五、十六”→“教训一、二、三、八、九、十、十一、十二”→“方法族停止索引”。
...
当前还评估一个原图全输出可靠性基准候选，关注完整检测误差、旋转几何和切片合并选择；实际仅完成文献、合同反例及本项目已有输出核验，未产生基准性能结果。
...
- 边界：这是错误适配的反例，不是 SeqCRC、CRC 或 LTT 理论的反例；也不否定合法非单调控制或 OBB 应用价值。一种保守界过宽不证明所有认证不可能；尚未开展真实基准实验。
