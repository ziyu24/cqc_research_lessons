...
审计基线：当前主线 `ziyu24/D7_pcp_obb_foundation@c8bc18bb70e5c0d17ac42830ae5a93b68ae82255`。已检查全部远端分支，没有更新更晚的次线。最新证据包括已完成的区域整数计数原型、逐图统计复核及几何无关的计数容量下界；主原型的方向/局部配额组合失败，global 简化臂的开发效率优势保留为未确认线索，不等于预注册主方法通过。

快速阅读路径：先读“项目研究什么”→“教训一、二、三、八、九、十、十一、十二”→“方法族停止索引”。
...
- **事实与未知**：global 简化臂的开发宽度优于整数运输，90% 名义覆盖时改善约 32%/16%；但缺其对应无方向对照与独立验证，不能将消融赢家自动提升为预注册主方法成功或新颖性证明。
...
- 边界：结论仅针对固定输出、类别不变移动、局部容量定义及开发样本；不证明所有几何或区域计数方法无效。global 在同一开发集比运输更紧，但尚缺对应方向消融和独立确认；事后选择它不能算原主方法通过。此处净计数差也不是经实例配对确定的 FN 数。
...
| OBB-local scene-count flow | 方向消融无增量、局部配额抬高 q；计数缺口在多数配置决定几何无关分位下界 | 停止固定主原型，不改阈值/网格/尺度/数据族挽救；global 的开发信号保留但不自动升级为新方法通过 | `doc/r013-b-scientific-review.md`；`doc/r013-b-image-weighted-review.json`；`src/r013_result_review.py` |
