...
审计基线：当前主线 `ziyu24/D7_pcp_obb_foundation@47becab275dac7619f23622e7fb2e7b0a887e34a`。已抓取并检查全部远端分支，没有更新更晚的次线。最新固定原图合并探索已完成；示例预算下经验与 HB 筛选均为空，平均作用较小但局部正例存在，独立部署条件仍未证明。项目按用户要求结束，不把当前资源决定解释成所有 OBB 可靠性研究不可能。此前十八条教训保留，本次仅新增示例预算与效应汇总的解释边界。

快速阅读路径：先读“项目研究什么”→“教训十九、十七、十八、十三、十四、十五、十六”→“教训一、二、三、八、九、十、十一、十二”→“方法族停止索引”。
...
最后评估原图全输出可靠性基准候选，先完成文献、合同反例和缓存核验，再收窄为固定内部检测器输出后的跨 tile 合并探索；已有两检测器、458 张历史原图、三个合并设置的开发结果，但未完成独立外部基准确认。
...
- **事实与判断**：有审核前合法计数上界时，主动 signed 盘点可以仿射变换成有界加权有限总体均值，保留可预测抽样、观测和成本；已有 weighted WoR 审核框架支持相应有效性。当前没有额外的空间效率机制或精确核验流程，主动提议未通过新颖性审查，未开展真实数据实验；被动 signed 的可识别耦合也尚未形成。
- **事实与判断**：固定矩形 ROI 的最大标准化计数误差可精确写成已有多输出同时共形带；同尺度查询族包含给出有效但一般性的分位/区间包含，不保证严格收益，也不能跨尺度继承旧运输支配。小型合成检查有严格正例及运输双向反例；未检验真实数据实用性。旧匹配成功图派生切分不能直接支持无条件整图计数。
- **事实与未知**：当前两类检测器在 DOTA/DIOR 的原图身份可闭合，但所保留筛分阶段与类别信息不同。严格反例否定从被筛分输出普遍复原 NMS 反事实、或把 GOSPA 总损失直接移植到单调 CRC 的假设；不否定原方法，也不证明基准无应用价值。
- **事实与未知**：固定合并探索的示例预算下，两检测器最小经验 FN75 分别为 36.47%/43.58%，已超过示例 30%；HB 筛选同样为空。合并阈值 .1→.5 的全图等权 FN75 改善仅 0.182/0.234 个百分点，但拥挤图有明确局部改善。两种事实都不支持普适无效或独立机制结论。
...
最新研究在真实实验前审查“随机核验少量影像块”的主动提议：推导 signed 残差与有界加权均值的仿射关系，构造重要性加权非负鞅作合同检查，穷举 72 个小型总体、两策略的 3,456 条完整路径；同时证明抽象隐藏质量困难族中的成本—认证率界。未提供新生产审核器，也没有用测试 GT 模拟结果冒充人工部署证据。

后续查询族提议直接校准全部矩形计数差异。实际完成的是与多输出共形带的响应变换、同尺度序关系证明、6,561 对小型整数直方图及精确秩/取整检查，并交叉核对标准化全子集基线的排序前缀计算；没有运行 HRSC/DOTA 新试验、性能基准或连续矩形算法。

随后以精确有理数构造六组检查，核对加框损失单调性、NMS 缓存信息、类别向量、风险保证层次、归一化代价及不同置信界的适用条件；只读核对已有预测的字段与原图 ID。未复现外部基线实验，未新增训练或推理。

最后固定 score≥.5、原图合并 NMS .1/.3/.5 和一次 229/229 calibration/probe 切分，重放 ORCNN/ARS 缓存；以原图为汇总单位报告三项 FN/FDP 风险、清单长度、Pareto 与示例预算筛选。独立标准库脚本核对 2,748 条充分统计、36 个 HB 上界及局部案例归类；没有新增训练、推理或把开发切分升级为独立部署验证。
...
## 教训十五：主动查询和有符号目标未必改变已有有限总体推断问题

- 失败命题：把预测后的区域差值改为随机审核少量空间单元，并允许不等成本、非均匀无放回抽样和任意停止，就已经构成严格的新统计机制。
- 失败原因：若 `0≤y_i≤K_i` 的上界审核前合法已知，则 `d_i=a_i(y_i−m_i)` 落在已知 `[ell_i,u_i]` 且跨度为 K_i。令 `f_i=(d_i−ell_i)/K_i`、`pi_i=K_i/ΣK_i`，总残差就是 `Σell_i+(ΣK_i)Σpi_i*f_i`，归约到有界加权有限总体均值。已审 y 与 f 可相互重建，因而可保留每一步条件抽样概率、停止时刻及累计成本。已有 UAI 2023 weighted WoR 审核框架也含侧信息和控制变量；当前提议没有另给新的空间策略或严格成本效率结果。
- 后续做法：在启动数据试验前，核对估计量、信息过滤、抽样概率与损失是否可一同变换到已知问题。把统计有效性与实际信息/成本改进分开；若后者才是贡献，须给出具体可部署机制及同信息强基线，不能用新应用名称替代。审核单元计数上界不能用未审核真值的最大值替代，理想 GT oracle 也不能代替真实核验合同。
- 边界：这是当前统计骨架的归约，不证明所有未来空间策略、误差核验者模型或人工审核应用没有价值。固定可见信息的隐藏质量困难族满足 `认证概率≤alpha/(1−已审负侧比例)`，但不是所有真实遥感分布的统一成本下界。小型精确穷举验证条件期望及一个数据依赖停止规则，不等于真实数据有效性或新颖性通过；不同保证层次的渐近 PPI 也不能仅凭标题当作有限样本反证。
- 证据：`ziyu24/D7_pcp_obb_foundation@0eab5d26d7b6900a10e486cec3a69d49ac52d0c0` 的 `doc/2026-09-07-b-review-of-c-spatial-audit.md`、`src/spatial_audit_l0_check.py`、`doc/2026-09-07-spatial-audit-l0-evidence.json`、`lab/discussion.md`；一手近邻为 [Shekhar 等，UAI 2023](https://proceedings.mlr.press/v216/shekhar23a.html)。

## 教训十六：查询合同细化和有效包含命题不自动构成新方法

- 失败命题：对未来整图给出全部预注册 ROI 的双侧整数计数区间，允许事后选择 ROI，再证明查询族缩小时区间不变宽，就已经超出已有共形方法，并支配原运输基线。
- 失败原因：把每个 ROI 当作坐标，将响应和预测分别除以仅依赖输入的正尺度，再使用常 modulation 的多输出同时共形带，分数、合法秩和区间与候选逐点相同。查询族包含只使 max-score 和同秩分位单调，不保证严格改善。查询依赖 sqrt 尺度不同于原整图公共编辑预算；合法合成例中候选为 `[6,12]`、单位运输为 `[8,10]`，另一例方向相反，故旧支配不能直接移植。
- 后续做法：新颖性比较同时检查响应变换、交换单位、预测依赖尺度和完整输出；同信息基线还须承担同一查询合同。保留正确包含关系及严格合成正例，另行证明新增信息、算法或应用效用。多套各自有效的查询带不能自动共同有效，整数端点也不等于任意回答组合满足加法；不能用匹配成功样本的旧切分保证无条件整图计数。
- 边界：未查到逐字相同的遥感应用不是首创证明；通用方法归约也不否定应用研究价值。这里只停止当前没有额外贡献的候选，未证明真实数据上无收益。连续矩形仍可构成 sup-score 索引，连续化本身不摆脱统计归约；新的计算或部署机制需单独评估，不能预判期刊等级。
- 证据：`ziyu24/D7_pcp_obb_foundation@fa448ea9498d8c21cea4560d40fffa8c049623b2` 的 `doc/2026-09-07-b-review-of-c-query-family.md`、`src/query_family_l0_check.py`、`doc/2026-09-07-query-family-l0-evidence.json`、`src/r009_p1_freeze.py`；一手方法见 [Diquigiovanni 等多输出同时共形带](https://arxiv.org/html/2106.01792) 的 §2、§3.1。

## 教训十七：统一风险名称不能替代原生损失与保证条件

- 失败命题：把整图检测误差统一称为有界 GOSPA 风险，就可直接移植各共形方法并在同一名义水平上比较其理论有效性。
- 失败原因：GT={0}、依次增加预测得到空集、{0}、{0,2} 时，cutoff=1、p=1、alpha_G=2 的 GOSPA 为 .5、0、.5，截断到 [0,1] 仍非单调。控制边际平均风险也不等于以 .95 概率对校准集条件风险认证：10% 校准集风险为 1、其余为 0，平均为 .1，但风险不超过 .1 的概率仅 .9。归一化和“输出全部候选”的后备动作又会改变 FP 代价，不能只按函数名继承定理。
- 后续做法：逐方法写明交换单位、信息、损失及单调/稳定性条件、风险随机性和有效性层次。有限候选可使用已有 LTT 等适用框架，但须允许认证集合为空；共同效果比较另定统一评价合同，不能改变原生保证。报告实际 FP/FN 和归一化效用，选置信界时核对损失假设。
- 边界：这是错误适配的反例，不是 SeqCRC、CRC 或 LTT 理论的反例；也不否定合法非单调控制或 OBB 应用价值。一种保守界过宽不证明所有认证不可能；该合同审查当时未开展真实基准实验；后续固定合并探索见教训十九，不能代替完整外部基准确认。
- 证据：`ziyu24/D7_pcp_obb_foundation@7dd70a32696510674ce04afecd51a214b2a96e38` 的 `src/benchmark_contract_review.py`、`doc/2026-09-07-benchmark-contract-evidence.json`、`doc/2026-09-07-benchmark-literature-and-execution-review.md`；原生条件见 [SeqCRC](https://arxiv.org/html/2505.24038) 与 [LTT](https://arxiv.org/html/2110.01052)。

## 教训十八：筛分后的检测输出不是任意后处理反事实的充分输入

- 失败命题：保存完整原图的 boxes/scores/labels，就足以恢复不同 NMS 阈值和完整类别集合基线。
- 失败原因：等高零角度框 A=[0,2]、B=[1,3]、C=[1.5,3.5]，分数递减；原候选在 NMS .5 保留 A,B，直接 .3 保留 A,C，而对前者缓存重做 .3 只留 A。即使阈值更严格，丢失候选仍不能恢复。另有类别向量 (.6,.3,.1) 与 (.6,.1,.3) 拥有相同 top-1，却在 .2 阈值得到不同类别集合；输出身份完整不代表决策所需信息完整。
- 后续做法：按反事实和算法需求记录实际保留阶段、候选身份、全类别分数及筛分规则；不能用后筛分矩阵冒充原候选，也不能用单列分数冒充类别概率向量。query×class 展开保留共同 query 身份。变更 tile 布局需要相应观测或明确重新推理，不能从旧图块坐标平移生成。
- 边界：可固定检测器内部处理，在真正保留的跨 tile 候选上重新研究跨 tile 合并 NMS；上述反例不禁止这种收窄。ID 闭合、GT 几何、ignore 语义、选优与父场景独立性是不同事实。仅完成缓存核验不能据此宣称新 benchmark 有效或无效。
- 证据：`ziyu24/D7_pcp_obb_foundation@7dd70a32696510674ce04afecd51a214b2a96e38` 的 `src/benchmark_contract_review.py`、`doc/2026-09-07-benchmark-contract-evidence.json`、`src/benchmark_asset_inventory.py`、`doc/2026-09-07-benchmark-literature-and-execution-review.md`。

## 教训十九：示例预算的空集、平均差与局部正例必须分别解释

- 失败命题：一个示例预算下没有认证动作，就证明所有部署合同无解；或平均变化很小就没有局部作用，单个局部改善又足以证明新机制。
- 失败原因：固定 FN50/FDP50/FN75 预算 .2/.2/.3 下，两检测器最小 calibration FN75 已为 36.47%/43.58%，因此经验筛选也为空，不能只归咎保守置信界；但三个设置仍全部位于经验风险—清单长度 Pareto 前沿。全部 458 张历史原图等权、NMS .1→.5 时 FN75 仅降低 0.182/0.234 个百分点，而拥挤图 P1397 的 T75 从 445→458、402→467。总体均值、局部案例与预算可行性回答不同问题。
- 后续做法：同时报告固定候选的经验风险、有效上界、完整权衡和预算来源；区分经验越界与仅因界宽而未认证。案例选择规则与事后解释须显式标注，保留真实局部作用但不当作独立子群确认；不能读结果后放宽示例预算或扩网格来制造通过。
- 边界：示例预算不是已验证业务要求，经验越界不证明总体真实风险必然越界；小平均差不是零效应证明，局部改善也不自动形成新的方法贡献。当前仅有固定输出和开发切分，checkpoint 选优独立性与父场景 iid 尚未证明，不提供独立部署认证。
- 证据：`ziyu24/D7_pcp_obb_foundation@47becab275dac7619f23622e7fb2e7b0a887e34a` 的 `src/r016_result_review.py`、`doc/r016-b-review-evidence.json`、`doc/r016-b-scientific-review.md`、`lab/result.md`。
...
| passive signed / active spatial audit proposal | 被动信息内尚无可识别耦合；当前主动统计骨架可归约到已有加权有限总体审核，缺新空间/成本机制 | 不下发当前提议的真实数据 pilot；不否定未来改变核验信息或成本模型的具体研究 | `doc/2026-09-07-b-review-of-c-spatial-audit.md`；`src/spatial_audit_l0_check.py` |
| finite ROI query-conformal discrepancy | 预测标准化后与已有同时共形带相同；同尺度包含成立，但不能跨尺度继承运输支配 | 当前缺额外算法/部署贡献，不下发 pilot；保留有效性与严格合成正例，不作真实数据负判 | `doc/2026-09-07-b-review-of-c-query-family.md`；`src/query_family_l0_check.py` |
| direct total-loss conformal adapter | 总损失非路径单调，原生风险保证层次不同 | 停止直接同名移植，不否定原方法或合法非单调控制 | `src/benchmark_contract_review.py`；`doc/2026-09-07-benchmark-literature-and-execution-review.md` |
| post-filter counterfactual replay | NMS 筛分和类别压缩有不可逆信息损失 | 停止声称缓存支持任意内部 operator；固定内部输出的跨 tile 合并仍可研究，效用未知 | `src/benchmark_contract_review.py`；`src/benchmark_asset_inventory.py` |
| fixed cross-tile merge pilot | 示例预算经验与 HB 筛选均为空；平均变化小但局部正例存在，部署独立性未证明 | 结束当前固定探索，不改预算或网格挽救；不推断所有合同无解或一般效应为零 | `doc/r016-b-scientific-review.md`；`doc/r016-b-review-evidence.json` |
