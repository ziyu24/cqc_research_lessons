...
- 边界：只有一个按已有结果选择的成熟父态、单种子和一个窗口；不声称统计显著、普遍刷新有害或旧教师永远优于新教师，固定模型逐一删图的刷新减复用差始终为负（−0.8238至−0.1837个百分点），排除了任意单张图独自驱动符号；1000次配对整图重采样2.5/97.5分位为−4.8594/+1.0780，描述不同图像组成下的敏感性，不是训练随机性、总体95%保证或显著性检验。未识别差异原因，不能归因于监督噪声或陈旧性。继续旧监督相对暂停的2.1922个百分点正值保留；整个自适应分配路线及同质量总成本目标仍未知。
- 证据：`ziyu24/cqc_P24@af29514ed1563741ab52eb2c6d067dca74f68910`；`lab/result.md`、`lab/failed_methods.md`、`configs/action_skip.json`、`configs/action_reuse.json`、`configs/action_refresh.json`、`src/summarize_actions.py`、`src/interventions.py`。
