...
全部关闭无监督的简单策略有明显局部负证据；固定教师×规律半量的延长窗口出现局部正信号，但同质量总成本、跨种子稳定性、收敛速度或创新尚未成立。共同19200父态后三动作中，继续旧监督60.8289%，高于暂停58.6367%及换当前教师后固定60.2086%。较早12800共同父态四格中，刷新相对旧教师从16000正差变为19200负差；后续从当前教师全量的16000完整best同恢复继续3200步，保留当前教师59.6546%，回退旧教师59.1183%。这不支持固定的“旧/新总是更好”规则；排序差不能单独归因于当前状态或恢复路径。B已独立回读该窗口原始更新、验证、成本和八份完整权重，来源、恢复状态、真实四卡计算及保护全部通过；原始证据限制已解除。相对16000父模型，继续全量仅高0.3100个百分点，半量仅高0.0527；这促使先诊断固定模型差值的图像组成敏感性，不直接追加训练或采用提前停止规则。固定模型图像敏感性与训练稳定性、总体显著性仍不可混同。已抓取来源全部远端分支，仅有main，无更新更晚次线；不审计其他项目。
...
- 边界：这是固定HRSC、单种子和有限选定窗口的经验负证据。共同12800父态6400步窗口中暂停终点52.0781%，旧教师半量60.2546%；当前教师全量的16000完整best同恢复继续3200步时暂停57.3584%，keep59.6546%，局部差为2.2962个百分点。后一个窗口已由B独立核实原始记录与完整权重。不能据此证明所有阶段都必须全量无监督，也不否定选择性调度。训练恢复的多worker增强不承诺逐位重放；不把数值差声称为统计显著。
- 证据：`ziyu24/cqc_P24@b1e78376c5e72254eb13ff262f8c7f1555829791`；`lab/result.md`、`lab/failed_methods.md`、`configs/learning.json`、`configs/decision_full.json`、`configs/decision_skip_unsup.json`、`configs/early_skip.json`、`configs/tail_skip.json`、`src/interventions.py`、`src/summarize_tail_actions.py`、`src/audit_tail_actions.py`。
