...
最新同密度EMA对应控制与五份冻结学生的复用内部留出复核来源为`ziyu24/cqc_P21@aeffbd8a06405a18557cc77cce5a015f140cc5ea`；实现为`src/dior_teacher_budget.py`、`src/dior_teacher_holdout.py`、`src/dior_teacher_update.py`和`src/dior_teacher_runtime.py`，检查入口为`src/audit_dior_teacher_result.py`与`src/check_dior_teacher_holdout.py`。补证及边界见第十六条。来源全部远端分支已抓取，仅main；准确EMA相对冻结部分错配只高0.9542/1.4257双AP点，未达预设实用对应标准；它在已暴露的固定内部留出上仍相对固定U、存在EMA和共同源保留双AP优势。数量选配对存在EMA的大优势与在线U替换正结果仍保留，但不能全部归因准确逐图对应，也不证明稀疏化为唯一原因或两者等价。
...
- 对应与留出补证来源：`ziyu24/cqc_P21@aeffbd8a06405a18557cc77cce5a015f140cc5ea`；`lab/result.md`、`lab/discussion.md`、`lab/failed_methods.md`、`configs/dior_teacher_budget.json`、`configs/dior_teacher_holdout.json`、`src/dior_teacher_budget.py`、`src/dior_teacher_holdout.py`、`src/check_dior_teacher_budget.py`、`src/check_dior_teacher_holdout.py`及`src/audit_dior_teacher_result.py`。固定输入、完整初态、两个实际计算rank、十二次验证选优、两端点完整PR及EMA/续训状态独立重放一致；没有新置换、训练真位置或U数量。随后五份开发合法best均先冻结、各以两rank推理完整2923张历史内部留出图、再统一读固定GT：准确EMA为26.2912/26.1588，固定U为20.8548/19.8710，存在EMA为13.6723/12.4347，共同源为16.2801/15.4115（AP07/面积AP）；准确EMA对前三者均通过既定保留条件。这个留出集早已用于旧模型，故结果只是复用内部留出上的固定排序保留证据，不是新盲测、官方基准、跨seed泛化或新颖性证明；准确对应控制的开发集失败不被重开。
