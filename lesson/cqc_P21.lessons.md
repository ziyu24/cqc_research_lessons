...
- 失败命题：在同一候选池、每图每类数量容量、EMA-U、初始化和训练日程下，用无梯度教师预测重排L实例，可以同时相对旧强源与同预算静态选择达到预先规定的双AP实用改善。原检验从一开始就要求两类参照，不能事后改写成只比较旧强源。
...
- 边界：两臂均使用数量、EMA-U及冻结框/文本/掩膜先验，单seed且复用开发集；未达预设幅度不等于零效应；动态学生开发best为29.0618/28.3221双AP点，仍应保留为实际局部正结果。此经验失败不否定数量监督、教师细化或动态实例选择的一般可能性，也不否认相对旧强源的局部正增益，更不是泛化、新颖性或顶刊证据。
- 证据：`ziyu24/cqc_P21@ea05ad93f83f460c74ad81803db0bbf095c043f9`；`lab/result.md`、`lab/failed_methods.md`、`lab/discussion.md`、`configs/r047.recovery.json`、`src/dior_labeled_refresh.py`、`src/check_dior_labeled_refresh.py`及`src/audit_dior_teacher_result.py`。两臂各真实双卡12轮，2923张完整开发图的四端点原生完整PR、源/配对比较已独立只读复算并与原始汇总精确一致；共同初始化、12次原生选优、实际两rank计算与完整EMA恢复及四模型散列均独立核验。没有新训练、模型前向或内部留出/官方test读取。
